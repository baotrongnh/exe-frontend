      ...
      <div className="bg-card border-b border-border px-8 py-6">
        <div className="flex gap-3">
          <div className="flex-1 relative">
            ...
            <Input placeholder="Job title or keyword" className="pl-10" value={searchInput} onChange={handleSearchInputChange} onKeyPress={handleKeyPress} />
          </div>
          <Button className="bg-primary text-primary-foreground hover:bg-primary/90 px-8" onClick={handleSearch}>
            Search
          </Button>
        </div>
      </div>
        ...
        <div className="flex-1 p-8 overflow-y-auto">
                  ...
                  <Card className="p-6 hover:shadow-lg transition-shadow cursor-pointer">
                    <div className="flex items-start justify-between">
                      <div className="flex gap-4">
                        <div className={`w-14 h-14 rounded-lg ${getRandomColor()} flex items-center justify-center text-white text-xl font-bold flex-shrink-0`}>{getInitials(job.title)}</div>
                        <div className="flex-1">
                          <h3 className="font-semibold text-lg text-foreground mb-1">{highlightText(job.title, searchQuery)}</h3>
                      ...
                      <div className="text-right flex flex-col gap-2">
                        <Button className={appliedJobs.has(job.id) ? "bg-green-600 hover:bg-green-700 text-white" : "bg-primary text-primary-foreground hover:bg-primary/90"} onClick={(e) => handleApply(job.id, e)} disabled={applyingJobId === job.id || appliedJobs.has(job.id)}>
                        ...
                        {job.deadline && <p className="text-xs text-muted-foreground">Deadline: {new Date(job.deadline).toLocaleDateString("vi-VN")}</p>}
                      </div>
                    </div>
                  </Card>
                </Link>
          ...
          {pagination.pages > 1 && (
            <div className="flex items-center justify-center gap-2 mt-8">
              <Button variant="ghost" size="icon" disabled={pagination.page <= 1} onClick={() => setPagination((prev) => ({ ...prev, page: prev.page - 1 }))}>
                <ChevronLeft className="w-4 h-4" />
              </Button>

              {Array.from({ length: pagination.pages }, (_, i) => i + 1).map((pageNum) => (
                <Button key={pageNum} variant={pagination.page === pageNum ? "default" : "ghost"} className={pagination.page === pageNum ? "bg-primary text-primary-foreground hover:bg-primary/90" : ""} onClick={() => setPagination((prev) => ({ ...prev, page: pageNum }))}>
                  {pageNum}
                </Button>
              ))}

              <Button variant="ghost" size="icon" disabled={pagination.page >= pagination.pages} onClick={() => setPagination((prev) => ({ ...prev, page: prev.page + 1 }))}>
