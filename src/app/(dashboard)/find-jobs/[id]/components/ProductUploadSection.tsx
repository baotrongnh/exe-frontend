...
export function ProductUploadSection({ jobId }: ProductUploadSectionProps) {
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const maxFiles = 5; // Reduced due to larger file sizes
    ...
    const validFiles = files.filter((file) => {
      if (file.size > maxSize) {
        toast.showToast(`File ${file.name} exceeds 100MB`, "error");
        return false;
      }
      return true;
    });
  ...
  const handleUpload = async () => {
    ...
    console.log("=== PRODUCT UPLOAD SECTION DEBUG ===");
            ...
            {products.map((product) => (
              <Card key={product.id} className="p-6">
                ...
                <div className="space-y-2">
                  ...
                  {product.files.map(
                    (file: ProductFile | string, index: number) => {
